...
public class StandardIVRResponse {
    ...
    private static KookooIVRResponseBuilder signatureTune(String callId) {
        return new KookooIVRResponseBuilder().withSid(callId).withPlayAudios(WHPIVRMessage.SIGNATURE_MUSIC);
    }

    private static KookooIVRResponseBuilder endOfCallTune(String callId) {
        return new KookooIVRResponseBuilder().withSid(callId).withPlayAudios(WHPIVRMessage.END_OF_CALL);
    }
}
